...
import {GetMixpanelId} from "../config";
...
class MixpanelClass {
  public awaitReady = async () => {
    ...
    while (!window['mixpanel']) await Sleep(2000);
  };

  public init = autorun(async () => {
    // @ts-ignore: window variable
    if (!Cookies.allowed || window['mixpanel']) return;
    ...
    // @ts-ignore: window variable
    while (!window['mixpanel']) await Sleep(200);
    // @ts-ignore: window variable
    await window['mixpanel'].init(GetMixpanelId());
  ...
  public track = async (event: string, data: any) => {
    ...
    // @ts-ignore: window variable
    await window['mixpanel'].track(event, data);
  };
  public alias =  async (alias: string) => {
    if (!Cookies.allowed) return;
    await this.awaitReady();
    // @ts-ignore: window variable
    await window['mixpanel'].alias(alias);
  };
...
async function campaignParams() {
  ...
  // @ts-ignore: window variable
  await window['mixpanel'].people.set(params);
  // @ts-ignore: window variable
  await window['mixpanel'].people.set_once(first_params);
  // @ts-ignore: window variable
  await window['mixpanel'].register(params);
}
